...
| **Attribution basis labelled per quote** | `speaker-labelled` / `single-speaker` / `turn-inferred`, from `/slava:disagreement:positions` | `turn-inferred` on a multi-speaker source is a STOP — drop the quote or supply a single-speaker source. |
