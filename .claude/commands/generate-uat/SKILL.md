...
7. **Output** — Save to `features/{spec-name}_acceptance_tests.md`
...
- Categories: {list}
- Output path: `features/{spec-name}_acceptance_tests.md`

Ask: "Does this look correct? Should I save the UAT file?"

If approved, save to `features/{spec-name}_acceptance_tests.md` (or custom path if `--output` specified).
...
|------|---------|-------------|
| `--output <path>` | `features/{spec-name}_acceptance_tests.md` | Custom output path |
