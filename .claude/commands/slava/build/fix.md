...
2. **If `reproduce_artifact` exists:**
   ...
   - **Re-check `fix_shape` against what you read, don't trust it.** It was written at `reproduced_at`, possibly days ago, and unlike `post_fix_timeout` no later step validates it. If `git log --oneline --since={reproduced_at} -- <spec path> <source files>` shows the spec or the touched files moved since, or the named alternatives have since been decided in the spec body, re-derive the call and say which way it changed.
