...
3.6. **Deploy manifest check** — run `./scripts/check-deploy-manifest.sh --env prod`.
   ...
   - Drift detected → **STOP**. Route by drift type + branch location:
     - **Migration drift while on a worktree feature branch** (the common case — `/dev` defaults to worktrees): do NOT migrate before merge (`stamp-deploy-manifest.sh` refuses to run from a worktree, and migrating from main pre-merge dirties main's manifest → a guaranteed cherry-pick conflict). Route merge-first: continue through the merge (step 3.7), and **do not migrate prod here at all** — `/push` owns it (P1211): its step 2.5 runs `check-schema-ready.sh` against prod's ledger and applies exactly the migrations the pushed SHA carries (`migrate.sh --only`), step 6 applies coupled ones after the deploy, and `git-ops.sh push-docs` / `ship-to-prod` plus the `schema-ready` required check refuse any push whose migrations are not on prod. *(Corrected 2026-09-21, P1211: this bullet used to say "push stays held, so there is no code-without-schema window" — that was prose for an agent to follow, and no script enforced it; on 2026-09-18 a `/push` in another session shipped a client ~10 min ahead of its migrations.)*
...
5. **Ready to push** — print:
   ```
   Ship complete. Ready to push: run /push
   (it applies any migrations this carries to prod first, then drives the staging hop;
    Vercel auto-deploys when main moves)
   ```
   *(Corrected 2026-09-21, P1211 / Codex review #15: this used to print `git push origin main`. A direct push to `main` is refused by the required status checks — nothing runs them on a SHA pushed straight to `main` — and it skips the schema gate entirely.)*

6. **Post-push prod-health watch (P866)** — when `e2e/prod-health-smoke.spec.ts` exists. After "Ready to push", the user runs `/push` (never auto-pushed by `/ship`). Once the user **confirms the push** (this skill never auto-detects it — it asks):
...
- **Push requires explicit user action.** `/ship` prints "Ready to push" and stops. The user runs `/push` when ready (P1211: never a raw `git push origin main` — GH013, and it skips the schema gate). Vercel auto-deploys on push.
