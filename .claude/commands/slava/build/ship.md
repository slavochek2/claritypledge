...
- **Prod migrate is NOT pre-approved** — `./scripts/migrate.sh --env prod` has its own blast radius (schema changes, RLS). Always gate it separately. The script enforces both P887 gates structurally: it prints the full pending-migration list and refuses to apply without ack (interactive `y` or `--yes`), and after any successful prod apply it auto-runs the prod smoke test — a smoke failure exits non-zero and signals possible schema-ahead-of-client breakage (P886 class).
