...
### 10. Validator parity canary (P839)
If the spec adds or modifies a file under `supabase/functions/` AND defines or references a validator predicate (named function matching `is(Valid|Allowed|Acceptable)*`, a constant ending `_REGEX`, a constant matching `ACCEPTED_*` or `VALID_*`, or any explicit numeric/length bound described as a server-side check), the spec's `## Test Coverage Strategy` section MUST list at least one parity canary file at `src/tests/p{N}-parity-*.test.ts` per validator. If the section omits this even though the trigger fires, emit:

`BLOCK: validator parity canary missing — see P839 rule. Worked example: src/tests/p835-reproduce.test.ts. The canary must (1) import the live client constant the validator's payload depends on, (2) embed a verbatim copy of the server predicate, and (3) assert every reachable client value passes. Generated by /generate-tests if invoked; if tests were written manually, the canary must still be present.`

This catches the case where tests were written manually (bypassing `/generate-tests`) or where `/generate-tests` ran before the P839 rule landed. The rule is the safety net — `/generate-tests` is the generator.
