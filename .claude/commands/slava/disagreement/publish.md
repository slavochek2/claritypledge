...
| **The audio check gates TEST too — do NOT scope it to prod** *(attempted and REVERTED 2026-08-31)* | unchanged: every quote, both environments | **"Test" is not private, and that is the whole answer.** This gate was narrowed to prod-only for one hour on the reasoning that `docs/decisions.md` 2026-08-19 made the publish boundary *environmental* (*"the founder reads the quotes and edits there"*), so test must be a private staging surface. **That inference was never checked and is false.** Verified: this skill writes every story `visibility = 'public'` (Stage 4 asserts it); the RLS policy `Stories readable by visibility` is `USING (visibility = 'public' OR author_id = auth.uid())` with **no environment term** (`20260209_add_story_visibility_enum.sql`); and this skill hands back a test-host feed URL and says to open it. So an anonymously readable web page carrying a real person's name is exactly what a test run produces. **A caption error reaching test has already done the harm — it does not need to be promoted to prod first.** The 2026-08-19 entry separates test from prod; it never says test content is private. Do not re-narrow this without first changing what test publishes or who can read it. |
...
| **The Management API token is enrolled in the keyring** — `PROD_SUPABASE_ACCESS_TOKEN` for prod, `SUPABASE_ACCESS_TOKEN` for test | `./scripts/keyring.sh status` shows it `enrolled` (never prompts) | The token is read through the per-access lock at Stage 5, never from `.env.prod` or `.env.local` plaintext (P1239/P1214). Without it the atomic path is unavailable — and since there is no fallback, its absence is a STOP rather than a silent downgrade. |
...
| **Attribution basis labelled per quote** | `speaker-labelled` / `single-speaker` / `turn-verified` / `turn-inferred`, from `/slava:disagreement:positions` | `turn-inferred` is a STOP — drop the quote or supply a source whose speaker can be confirmed. **`turn-verified` is filable, but only with its per-quote confirmation artifact present** (which of the three confirmations landed, and the confirming text). A `turn-verified` label with no artifact behind it is a `turn-inferred` wearing a better name — treat it as the STOP. **`speaker-labelled` is filable only with a non-empty `diarization` block in the run file** — all four of `oracle`, `turns`, `speakers`, `mapping_evidence` present, and `mapping_evidence` quoting an actual line from the source that fixes the label to the person. A `speaker-labelled` label with a null, absent or unquotable `diarization` block is `turn-inferred` wearing the BEST name — treat it as the STOP. **Added 2026-08-28 after an adversarial review found the stronger basis was receiving LESS scrutiny than the weaker one:** this row demanded an artifact for `turn-verified` and nothing at all for `speaker-labelled`, so an agent could write `speaker-labelled` with a fabricated block and pass every downstream check. Nothing in `scripts/` parses run files (verified: `grep -rn 'gate_0_basis|points-runs' scripts/` returns nothing), so this prose is the only thing standing there. |
...
**2. Derive the target ref from the ROW OF THE ENVIRONMENT TABLE matching this run's target — and name the target FIRST.**
**TEST ⟹ `.env.local: VITE_SUPABASE_URL`. PROD ⟹ `.env.prod: VITE_SUPABASE_URL`.** Read credentials from `.env.local` in both cases, by variable name, from the same row. Never merge the two files into one environment. Print the target name and the ref before the first write so the environment is visible.

> **Corrected 2026-09-01. This constraint read "Derive the target ref from `.env.prod` ONLY" and it was the most dangerous line in the file.** It was written from a prod-run's point of view — the real hazard it names is genuine (*`.env.local` overrides `VITE_SUPABASE_URL` with the test ref, so a PROD run that reads `.env.local` silently lands on test*) — but stated unconditionally it inverts on the mandated **test** run: an agent told to publish to test follows it, reads the **prod** ref, and either writes to production while labelling the ledger `env:test`, or fails on a mismatched ref/key pair. **A skill that requires a test run first must be executable on a test run.** The environment table has always had this right; three separate places said otherwise (this constraint, Stage 1, and the quality checklist) and all three are now environment-selected. The original hazard is preserved by the row rule: a prod run still may not take its URL from `.env.local`.
...
stories         one per arguer
                  ...
                  content    = the agent's summary + the "Supporting quotes from {Full Name}"
                               label + "#<event-tag>"
                               NOT the quote bodies — P1212 §1 moved those to
                               video_quotes, which is the only place they live.
                               NOT a source link — that is video_url.
                  visibility = 'public'
...
- [ ] **Target ref came from the row of the environment table matching THIS run's target** — `.env.local: VITE_SUPABASE_URL` for **test**, `.env.prod: VITE_SUPABASE_URL` for **prod** — credentials from `.env.local` by variable name, the two files never merged, and the ref printed before the first write. *(Corrected 2026-09-01: this line read "Target ref came from `.env.prod`" unconditionally, which **no compliant test run could ever tick**. The environment table above has always had it right. The failure was asymmetric and pointed the wrong way — an agent following the checklist literally sends a run it believes is "test" to the PROD ref, which is the exact incident shape this file's own environment-table section was written to prevent.)*
...
- [ ] **Each agent's avatar EITHER returned `200` + `image/*` from the URL read back from the target ref, OR is a deliberate absence** — `avatar_url IS NULL` **with** its written registry line, which takes the initials-only path and is probed not at all. *(Corrected 2026-09-01: this item demanded a `200` for every agent unconditionally, which **no deliberate-absence run could tick** — it contradicted the hard precondition above, which explicitly branches on `NULL` and forbids a probe on that branch. Pre-existing; found by adversarial review during P1202.)*
...
- [ ] **Every story with a video carries `Supporting quotes from {Full Name}` verbatim in its text**, verified from the read-back, not from the text you wrote. A mechanical backstop mirroring the hashtag check above — this skill does NOT author the rule. The voice rules and the label live in `/slava:disagreement:story-draft` and nowhere else; wrong text ⟹ re-run disagreement:story-draft. **The quote BODIES must NOT appear in `content`** — P1212 §1 put them in `video_quotes` alone, and a body found in the text will render on the page twice.
- [ ] **No story carries the label with zero quotes under it.** For every row whose `content` holds `Supporting quotes from`, assert from the read-back that `jsonb_array_length(video_quotes->'quotes') > 0`. *(Added by P1212 §1. The label check above is a bare string grep and the per-quote checks are universally quantified over `video_quotes.quotes` — **vacuously true on an empty array**. So an arguer whose every candidate quote was dropped upstream at `/slava:disagreement:positions` Step 4b, but who still has a `video_url`, passed every check and published a story reading "Supporting quotes from {Name}" with nothing beneath it. The quote block renders nothing on an empty array, so the heading was the only thing left. This hole pre-existed §1; §1 made it the visible default rather than a masked one, because the bodies are no longer inline to fill the gap. `story-draft` is told to omit the label in that case — this is the backstop for when it doesn't.)*
...
- **Not a promoter.** There is no copy-from-test operation *in this skill*. **When the live test rows have been reviewed and improved in place — the normal case after any read — the reviewed bytes are in the database and this file's `## Story Drafts` section is stale; use `/slava:disagreement:promote-to-prod`, which takes the test rows as its source and refuses without independent accuracy evidence bound to those exact bytes.** Use this skill's own second invocation only for a prod filing where the run file IS the reviewed artifact. *(Corrected 2026-09-09. This line previously read as an unconditional ban on promotion, and following it literally would have filed the pre-review draft to prod: all 8 stories differed, and the run-file copy of one still carried three fabrications about a named living person, still marked `[HELD]`. The seals do not cover the story section — verified by recomputation — so the run file's story text holds no authority the database rows lack.)*
