...
```
DETECTED: ‹N› candidates · subject ‹X› · excluded ‹Y› · corpus ‹path› (‹size›, read in full | partial: ‹how›)
...
- [ ] **Anchors re-matched against the corpus.** `grep -F` at least three `evidence` strings against the corpus file; all must match exactly. **This is the gate that catches an undecoded UTF-16 read** — interleaved spacing makes every quote unmatchable while the verbatim check above still passes on inspection. State the encoding and the decoded byte count in the summary. Any miss ⟹ re-decode and re-run; do not print cards.
