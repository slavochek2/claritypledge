...
- When `status: qa` → feature is implemented, awaiting user review before shipping. Stays on feature branch; run `/ship pN` to merge to prod. **Hard gate:** before setting `qa`, all `## Acceptance Criteria` checkboxes must be `[x]`. If any `[ ]` remain, do not set `qa` — report the unchecked items. Enforced in `/fix` (Feature QA Gate step 0) and `/dev` (UAT gate step 1).
