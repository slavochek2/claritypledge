...
## Hard Stop — Run /slava:maintain:claude-md First

**Before making any edit to `CLAUDE.md` or any file under `.claude/rules/`**, run:

```
/slava:maintain:claude-md "description of what you want to add or change"
```
...
**Do NOT proceed without running /slava:maintain:claude-md first.** This applies even when:
...
## What /slava:maintain:claude-md checks
...
Subagents (spawned via Agent/Task tool) must **NEVER** edit `CLAUDE.md` or `.claude/rules/*.md`. These files may only be edited by the main conversation agent after running `/slava:maintain:claude-md`. Subagents may propose changes as terminal output — the main agent applies them through the gate.
