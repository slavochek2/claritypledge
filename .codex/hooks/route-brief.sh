...
# (3) every target here (/status, /pick-flow, and the Codex routing adapter) is a cp
...
# derived from prompts the founder actually typed in historical harness transcripts
...
#
# Rollback: remove the UserPromptSubmit entry from .codex/hooks.json (or delete this
...
LOG_DIR="${ROUTE_BRIEF_LOG_DIR:-$HOME/.codex/logs}"
...
- MODEL / EFFORT ASK -> give the call in one line, format `<task type> -> <model>,
  <effort>`, per ~/.agents/model-routing.md and ~/.codex/model-routing.md, plus one clause of why. Honest
...
- COMPACT / PLAN-MODE / SUBAGENT ASK -> answer concretely, never deflect back to the
  ...
  other side (AGENTS.md, active spec, and last edited file). Plan mode or subagents: say which and
...
match_triggers() {
	...
	END {
		...
		for (i = 1; i <= n; i++) {
			...
			if (s == "delmo") model = 1
			# model names exposed by the Claude and Codex adapters
			if (w[i] == "haiku" || w[i] == "fable" || w[i] == "sonnet") model = 1
			if (w[i] == "sol" || w[i] == "terra" || w[i] == "luna") codexmodel = 1
		}
		...
		else if (effort && (opus || sonnet || codexmodel))   labels = labels " model"
