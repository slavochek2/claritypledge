...
import { verifyEndorsement, getProfile } from "@/polymet/data/mock-profiles";
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (profileId && witnessId) {
        ...
        if (target) {
          const newProfile = verifyEndorsement(profileId, witnessId);
          if (newProfile) {
            setEndorserProfile(newProfile);
        ...
        <div className="border-2 border-border rounded-lg p-8 bg-card space-y-6">
          <div className="space-y-2">
            <h3 className="text-2xl font-bold">
              Your Pledge Profile is Ready! 🎉
            </h3>
            <p className="text-muted-foreground">
              We've created a profile for you. Now you can take the pledge
              yourself and build your own network of endorsers.
            </p>
          </div>
          ...
          <div className="bg-muted/50 rounded-lg p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button asChild variant="outline" size="sm">
                <Link to={`/p/${endorserProfile?.id}?owner=true`}>
                  View Profile
                </Link>
              </Button>
            </div>
