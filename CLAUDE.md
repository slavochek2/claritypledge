...
**Name the skill you're running:** When informal language maps to a skill ("simplify this" → `/slava:build:simplify`, "what now" → `/status`, "where are we?" → `/status`, "anything to kdd?" → `/kdd`, "wrap up" → `/wrap`), invoke the skill and name it — so the user learns the command exists.
