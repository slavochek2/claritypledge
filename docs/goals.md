...
> **[ACTIVE 2026-09-21 — founder decision, `UNTESTED`, zero pilots run] Champion-hosted pilots in mid-size organizations (~100–1,000 people).** Supersedes the event-led block below as the **conversion** path; the structural bet is in [lean-canvas.md](lean-canvas.md) §Customer Segments + §Channels, and the reasoning in [decisions.md](decisions.md) 2026-09-21 [product]. **Motion:** find a champion → qualify (the 2026-07-29 throughput trigger + the abandoned-conversation question below) → agree on success criteria **before** the pilot (e.g. X% opt in, a named disagreement unstuck, the principle used in a real meeting within 2 weeks) → ask early who approves a follow-up → one free, personally prepared online pilot on the champion's stuck disagreement → discuss support only if the room shows adoption. **Before the first organization pilot:** 1–2 low-stakes online rehearsals with friends or community members, since the online format is untested. **Public physical events continue as the lab** (run-of-show fixes from event #1 below) and as champion sourcing. They are no longer the conversion channel, and the "one person per company" rule no longer governs the pilot room. **Falsifier:** 5 champion conversations with 0 pilots agreed ⟹ change the pitch; 3 pilots with no paid question ⟹ the segment is wrong (thresholds agent-proposed, `[FOUNDER DECISION: overwrite]`). **Still unreconciled below, deliberately:** the event-led block's per-lane rates, the membership ladder and the startup-coordinator multiplier ask (*"can I run this for your startups?"*) were built for the outgoing funnel and are kept as written until the pilot motion produces its own numbers.
>
