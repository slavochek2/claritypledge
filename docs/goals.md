...
**Before outreach (minimum):** a ladischenski.com page/blurb + booking link for the free session (divergent-AI hook, no public price). **cp front door is now in scope (supersedes the earlier "cp untouched", 2026-07-13 — [decisions.md](decisions.md)):** the stale co-founder surfaces (`/` ProgramPage + `/program` €950 offer) contradict the active key-hire wedge and are realigned in P987; removing a mission-contradicting asset is cleanup, not new-funnel build. The active *acquisition* surface stays ladischenski.com; the cp group rung stays deferred. Track the ladischenski build as a task in the `ladischenski-com` repo (no cp-kanban ticket — different repo).
