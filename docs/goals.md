...
**Core outcome (re-sequenced 2026-09-04):** run **event #1 first**, then the **multiplier campaign** (~15 coordinator conversations → rooms) — the multiplier ask *"can I run this for your startups?"* presupposes a run that exists, so it is empty until one does ([decisions.md](decisions.md) 2026-09-04 [product]). **Event #1 date: `[FOUNDER DECISION]`, not yet set** — the venue organizer is engaged (contact lives in chat, not here), and the founder sets the date when satisfied with the pipeline's point set; the named risk is that "working well" has no exit criterion reachable from the desk, so the room is the test. Then land the **first fixed-price install** for **H-BuildRightThing-Sell** — sourced through the free event, closed on a free 30-min scoping call, **€4,500 with €1,000 phase 1 taken at the event** (see the ladder above; the *target* segment is still `PROPOSED`, not settled). Revenue is a **bridge with a tripwire** (runway + the reps that *package* the product), not the mission ([decisions.md](decisions.md) 2026-07-11 [product]).
