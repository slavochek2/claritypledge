...
**Active test occasion (2026-06-19) — founding-cohort run:** the concrete test is now the founding-cohort launch ([goals.md](goals.md)) — coaches with a business partner (the coach-fit ICP) guest on the biweekly webinar and demo their own divergence live. This run is the **first co-delivery occasion feeding** the transform-if above (which needs ~3 co-deliveries to fully evaluate), not a one-shot evaluation of it. Funnel mechanics + band live in goals.md / [decisions.md](decisions.md) 2026-06-19, deliberately not duplicated here (unrun process stays out of the scoreboard).
