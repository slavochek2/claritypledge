...
| H-CoachChannel | Coaches carry the instrument as the PRIMARY distribution channel (coach-of-coaches first) | Active | P0 | — | Promoted 2026-06-02 — primary distribution after R₀≈0 killed the viral front door |
...
**Bet (2026-07-01, market-focus shift to founders):** The active market is **founders / solo AI builders** (single-person funnel, reachable, growing — chosen on reachability, not because only they need it). The wedge Problem is a **will** problem, not a skill one: a solo builder bets months on a belief no one stress-tested and **won't reveal the gaps in their own understanding when the truth might reject what they love** — so a peer's challenge lands as *attack*, a customer's politeness as *validation*, and the bet never gets tested. Value = verify-then-challenge (challenge made *usable* because understanding is verified first): **unlock the will → build the muscle gradually** (peers on far-off topics → own product → real customers) **→ de-risk the bet**. Confirms the canvas's will-primacy (5-move Move 1; H-AffectiveHonesty). The cohort/room is the plausible product (you can't verify-then-challenge alone), tested for demand by a 1:1 test.
