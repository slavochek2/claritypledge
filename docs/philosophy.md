...
1. **Degraded on the target of a live complaint — UNTESTED in work dyads.** Gottman's stated mechanism is that the protocol expects "emotional gymnastics from people who … are somewhat emotionally disabled by conflict." Derived from a romantic dyad mid-fight, where exit is costly and identity fused. A work team's version — *"I don't think this is what the customer asked for"* — is H-BuildRightThing's **modal case**, not an excluded one, so this must not be read as ruling out CP's own P0. **Novel prediction:** a measurable accuracy decrement when the listener is the target of the complaint versus a matched non-target listener, same dyad, same topic. **Falsifier:** teams paraphrase successfully on a live contested decision at no accuracy cost ⟹ the bound is couples-specific and drops.
2. **Not without pre-commitment.** Nothing in paraphrasing supplies the will to start paraphrasing — the CPA is signed while calm because the will problem is upstream of the skill (lean-canvas §Problem; H-AffectiveHonesty). The one bound that is independently supported and not an anomaly-absorber. **Novel prediction:** pre-committed dyads sustain protocol use at a rate non-pre-committed dyads at matched motivation do not.
3. **Prevention-vs-repair timing — UNTESTED, weakest of the three.** Derived from a non-randomized two-programme comparison (Munich therapy for distressed couples vs EPL prevention for calm ones) whose evidence this project **declines to bank** as self-selected ([decisions.md](decisions.md) 2026-08-03). H-TherapistRetention explicitly bets the other way — that pre-commitment lets the protocol be invoked *at* the rupture. Treat as a prediction to test, not a constraint. **Falsifier:** a pre-committed dyad invokes the protocol successfully mid-rupture ⟹ delete this bound.

A separate honesty bound, which does not depend on any of the above: **the postulate claims active listening makes falsification *possible*, not that it improves outcomes, and it should never be read as one.** Whether verified comprehension *mediates* outcomes is H-ComprehensionTrust's open question — not a settled negative (see [theory-of-change.md](theory-of-change.md) §Evidence Gaps for what the adjacent literature can and cannot establish).
