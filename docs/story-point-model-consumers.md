...
| `/slava:problem:submit` | **Splits** A → B → obstacle, where `/problemify` deliberately **welds** it — and drops A and B to story material, keeping three claims (frame · obstacle · hypothesis) | Opposite handling of the same frame, and both are correct for their job. `/problemify` welds because the trajectory *is* the diagnosis, for one person in the room. `problem:submit` splits because P1182 matches on the **slot**, and a welded frame gives a matcher nothing to match on. The split is licensed by the two-arbiter test, not by preference: A and B have private referents and become story; what survives as a claim is whether the written account supports the diagnosis. **Registered 2026-08-31** (`decisions.md` 2026-08-31 [product]) |
| `/slava:problem:submit` | Inlines `/slava:understanding:detect`'s elicitation procedure rather than calling it | `decisions.md` 2026-08-06 [process] — elicitation procedure is **not** shareable between skills; definitions and acceptance contracts are. Eliciting from an archive (can grep, cannot ask) is a different procedure from eliciting from a live human. **This duplication is the ruling, not drift — do not "fix" it by extracting a shared step** |
| `/slava:understanding:create-letter` | Files **two** points (point + anti-point), not the letter pipeline's three-element structure (fact point / anti-point / norm point), and inherits no emotion gate | An agent paraphrasing someone's reasoning has no feelings to elicit, and there is no shared event record to establish common ground with — so the fact point has nothing to be checkable against. **Registered here 2026-08-10** after an adversarial review found the skill claiming this divergence was "registered as such" while it appeared only in the flat pointer list below |
...
`docs/CHARTER.md` · `docs/definitions.md` · `.claude/commands/slava/disagreement/select.md` · `.claude/commands/slava/disagreement/prepare.md` · `.claude/commands/slava/disagreement/positions.md` · `.claude/commands/slava/disagreement/story-draft.md` · `.claude/commands/slava/content/sifter-definitions.md` · `.claude/commands/slava/content/sifter-point.md` · `.claude/commands/slava/content/sifter-story.md` · `.claude/commands/slava/think/align.md` · `.claude/commands/slava/understanding/detect.md` · `.claude/commands/slava/understanding/reconstruct.md` · `.claude/commands/slava/understanding/create-letter.md` · `.claude/commands/slava/problem/submit.md` · `features/p1012_reverse_story_sender_paraphrase.md` · `features/p1030_reverse_story_and_align_pipeline.md` · `features/uat/p1030.md`
...
`disagreement:prepare` points at the model doc and copies nothing from it — but it **introduces model-level concepts the model doc does not contain**, which is a third category this register had no row for. Not drift; a gap in the model:

| Concept | Where it lives now | Belongs in the model because |
|---|---|---|
| **The synthesized point** — a claim neither speaker made, built so each speaker's own quotes commit them to opposite ends | `disagreement:prepare` §4a | It is a *kind of point*, not extraction craft |
| **A position held by an agent** — what it means for a non-human to hold one, and what a reader must understand by it | `disagreement:positions`, `features/p1104_*.md` | The model defines who may hold a position; agents are not in it |
| **A story as evidence for a position** rather than as its author's lived experience | `disagreement:story-draft` | The model's Story is first-person experience; a quote bundle is a different object wearing the same name |

**Three model rulings still live in skill files rather than in the model** — split across `disagreement:prepare` §4a, `disagreement:positions` and `disagreement:story-draft` since the P1156 restructure moved two of them out of `disagreement:prepare` (§6 → `disagreement:positions`, §8 → `disagreement:story-draft`). The register tracks them wherever they live — the exact rot it exists to catch is the ruling drifting away from the model, not which skill file holds it today.
