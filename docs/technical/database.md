...
**RLS scoping rule (P1039, after the P1035 unscoped-service-role-bypass incident):** a new migration's `CREATE POLICY` for a non-SELECT command (`INSERT`/`UPDATE`/`DELETE`/`ALL`, or `FOR` omitted) whose `USING`/`WITH CHECK` clause is a literal `true` or a role-identity function (`current_setting('role')`, `auth.role()`) must carry an explicit `TO <role>` clause — omitting it silently defaults to `PUBLIC`, including unauthenticated. Public SELECT policies are never flagged (normal, common pattern). Exempt with `-- intentionally-public: <reason>` when the policy is deliberately open to all roles. Enforced by pre-commit via `scripts/check-migration-rls-scope.sh`. Regression canary: `src/tests/p1039-reproduce.test.ts`.
