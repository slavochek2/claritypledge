...
**Reserved account namespaces (P1104):** Two channels are reserved so a machine account can never be mistaken for a person, and a person can never wear a machine's identity. `is_reserved_agent_name(text)` reserves the `Agent · ` display-name prefix; `is_reserved_agent_slug(text)` reserves the `agent-` URL prefix and `is_reserved_machine_slug(text)` still reserves the retired `machine-` one. Both normalise with **NFKD**, strip invisible characters, fold confusables, strip the combining marks NFKD exposes, and then test the first token — the mark strip is load-bearing, because a combining mark is not `[[:alnum:]]` and would otherwise split the very token being tested (a confirmed bypass, closed by `20260824140000_p1104_slug_guard_decompose.sql`). Each is enforced at three points: the `guard_profile_trust_columns` trigger (SECURITY INVOKER, so it sees the client role), `upsert_my_profile` (SECURITY DEFINER, so the trigger's client branch never fires inside it and it keeps its own copy), and a positive assertion in `create_or_reuse_agent_account` — an agent account cannot be created without both markers. Migrations: `20260819160000`, `20260820091000`, `20260820092000` (name), `20260824120000`, `20260824140000` (slug), `20260904140000`/`20260904150000`/`20260904160000` (the `agent-` predicate), `20260904170000` (its call sites).

**P1212, 2026-09-04 — the namespace was renamed and the old one stayed shut.** `create_or_reuse_agent_account` now REQUIRES `agent-`; the trigger and `upsert_my_profile` refuse `agent-` AND `machine-` from clients. The asymmetry is deliberate: reserving both against clients costs nothing, while requiring both of an agent is impossible. A retired namespace is a more attractive impersonation target than a live one, because the links already in circulation point at it.

**Known gap, live since `20260824140000` and inherited by both predicates:** the combining-mark strip keeps `[:alnum:] [:space:] [:punct:]` and drops the rest — and Unicode classifies `~` as Sm (Math Symbol), not punctuation. So `machine~x` and `agent~x` tokenise as one word and are NOT reserved. Same for `+ < = > | $ ^` and backtick. `e2e/integration/p1212-agent-slug-reservation.spec.ts` asserts the current behaviour so a fix fails the test loudly instead of passing silently.
