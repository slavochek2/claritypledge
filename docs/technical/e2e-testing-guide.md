...
**StrictMode also drops a timestamp seek on a YouTube player that has not mounted yet** — dev only; the player's effect cleanup clears the pending seek before `onReady`. Assert "a timestamp plays on a cold player" against a production build, not `npm run dev`. See `docs/decisions.md` 2026-09-11 [technical], "In dev, React StrictMode drops the first timestamp click".
...
- `createAuthClientForUser(email)` — tests that need to call a Supabase RPC as a specific authenticated user (non-browser). See `e2e/p731-inbox-letter-arrival.spec.ts` lines 27–43 for the full implementation (signs in with `test-password-12345`, returns a client with the JWT set in headers).
