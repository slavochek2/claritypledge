...
- **Supabase CLI not linked in worktrees.** `supabase` CLI is linked to the main repo directory (via `supabase link`). Running `./scripts/migrate.sh` from a worktree fails with "Cannot find project ref." **Workaround:** Copy the migration file to the main repo and run `./scripts/migrate.sh` from there, or run `supabase link` in the worktree (creates a `.supabase` dir). If `migrate.sh` is itself blocked (db-push history drift on the shared test DB, and/or a stale keychain PAT shadowing `.env.local` → 401), see `docs/decisions.md` 2026-06-02 [process] (keychain-first PAT shadow) and 2026-06-02 [technical] (direct Management-API `database/query` apply — note: applying DDL that way leaves `supabase_migrations.schema_migrations` un-stamped, so re-run `migrate.sh` once unblocked to record history).
