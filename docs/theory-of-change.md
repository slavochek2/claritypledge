...
**Illusion of common knowledge — addition to Pinker (2026-03-28):** Pinker's framework has two states: shared and common. The genuine theoretical addition is a third: the *illusion of common knowledge* — people believe they have common knowledge but don't. This is not an edge case; it is the default for natural language comprehension. Pinker's examples of failed common knowledge are about absence (regime suppression, indirect speech). The ClarityPledge case is different: information was exchanged, both believe comprehension occurred, both are wrong. Four epistemic states for any story/point/position: (1) private, (2) falsely common (the dangerous one — both act as if aligned), (3) shared (both have it, unverified), (4) common (verified, recursive). The /live protocol detects state 2 and transitions to state 4. Boundary condition: shared-not-common is adaptive and sufficient for short-loop coordination (café meeting — error is immediately visible). It becomes pathological in long-loop, high-stakes coordination (co-founders, partnerships) where feedback is delayed, cost compounds silently, and error is invisible. This identifies the specific boundary where Pinker's framework needs extension, and explains why the co-founder pair is the natural first market.
