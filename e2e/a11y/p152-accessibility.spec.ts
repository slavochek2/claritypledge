...
test.describe('P152: Profile Calibration Accessibility', () => {
  ...
  test('empty state CTA button is keyboard accessible', async ({ page }) => {
    ...
    // Find CTA button
    const ctaButton = page.getByRole('button', { name: /start verification|find events/i });
    await expect(ctaButton).toBeVisible();

    // Tab to button
    await page.keyboard.press('Tab');
    await page.keyboard.press('Tab');
    // Keep tabbing until we reach the CTA
    for (let i = 0; i < 10; i++) {
      const focused = page.locator(':focus');
      const text = await focused.textContent();
      if (text?.match(/start verification|find events/i)) {
        break;
      }
      await page.keyboard.press('Tab');
    }
    ...
    expect(currentUrl).not.toContain(`/p/${testUser.slug}`); // Should navigate away
  });

  test('help icon tooltip is keyboard accessible', async ({ page }) => {
    // Setup: Create insufficient calibration data to show empty state with help icon
    await createCalibrationData({
      listenerId: testUser.user.id,
      speakerId: visitorUser.user.id,
      count: 2,
    ...
    // Tab to help icon (?) next to "Understanding Calibration"
    for (let i = 0; i < 10; i++) {
      const focused = page.locator(':focus');
      const ariaLabel = await focused.getAttribute('aria-label');
      if (ariaLabel?.match(/what is.*calibration/i)) {
        break;
      }
      await page.keyboard.press('Tab');
    }
    ...
    await expect(page.getByText(/knowing how well you understood/i)).toBeVisible();
  });
