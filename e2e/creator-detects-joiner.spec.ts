...
test.describe('Creator Detects Joiner', () => {
  test('Creator transitions to live view when joiner joins', async ({ browser }) => {
    ...
    try {
      ...
      await expect(joinerPage.getByText('Did you understand me?')).toBeVisible({ timeout: 5000 });
      ...
      await expect(creatorPage.getByText('Did you understand me?')).toBeVisible({ timeout: 5000 });
  ...
  test('Creator detects joiner via polling fallback if realtime fails', async ({ browser }) => {
    ...
    try {
      ...
      await expect(creatorPage.getByText('Did you understand me?')).toBeVisible({ timeout: 2000 });
