...
Rejected during `/reproduce`. Findings:

- **Not a code defect.** The Supabase JS client (`src/lib/supabase.ts`) uses `flowType: 'pkce'` with default localStorage-backed session persistence. No session cookie is set; the session lives in `localStorage` after `verifyOtp`. The `confirm-letter-response` edge function returns `{ ok: true }` and does not set Set-Cookie. App behaves as designed.
- **Cause is external.** Gmail in-app WebViews on iOS/Android spawn new WebView instances per link tap with isolated/ephemeral storage partitions. The session in localStorage is not reachable from the second WebView spawn. This is OS/embedder behavior, not an app bug.
- **No clean in-app fix.** Candidate mitigations (delivery_id in URL → server-resolved identity, banner nudge to re-auth, PWA install) are all product-level changes, not bug fixes. If pursued, they should be filed as `type: story` features with their own appetite.
- **Workaround exists.** Long-press share link in Gmail → "Open in Safari/Chrome" lands authenticated (system browser shares localStorage with the app).

If activation funnel data later shows this failure mode is material, file a follow-up `type: story` for the fallback-identity mechanism.
