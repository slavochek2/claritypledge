...
- [x] **The backfill cannot write a partial or wrong set** — reworded 2026-08-31 with founder approval, because the original wording asserted a fact about production that can only be observed AFTER a push, while `/ship` closes this spec BEFORE the push. What is verified now: the migration enumerates the 8 Chiang Mai slugs literally, raises a hard `EXCEPTION` if it touches more than 8 or if touched ≠ present, and asserts the 2 Ko Phangan rows stay NULL (`20260828164500_p1060_events_org_id.sql:88-98`, read and confirmed).
  **POST-DEPLOY FOLLOW-UP, NOT OPTIONAL:** immediately after the first prod push, query prod for the 10 named slugs and diff against this spec's two tables. UAT-1.2 SKIPPED on the test DB (it carries none of the prod slugs) — a skip is not a pass, and this is the irreversible half of the migration.
- [x] The · Online blurb (D7) is supplied by the founder and recorded in this spec before the seed migration runs — founder delegated the drafting and approved 2026-08-31. Blurb: *"Calibrated communication practice with people outside your own field — no local group needed."* Card subtitle: *"Practise with people outside your own field"* (replaced "The topic is set in advance", rejected by the founder as describing one event format rather than a community). About body set in `20260831160000_p1060_online_org_copy.sql`. **Why not "online" as the distinction:** · Chiang Mai's own About already reads "We meet in person and online", so the medium never separated the two groups — the difference is a local room and shared context vs neither.
...
- [x] The visual reference is founder-approved and its four open items are resolved, before the contract is pinned — founder reviewed and approved 2026-08-31 ("the screens look fine").
- [x] `/org` lists every public organization and links to each; readable signed-out; carries no create-organization affordance — matching the approved reference at 320px, 375px and desktop — the no-create-affordance and signed-out halves are asserted mechanically (p1060-source-contract + e2e); the width match is the founder's approval of the round-7 renders, 2026-08-31.
