...
3. **Provisioning cannot complete inside one run, because it spans a deploy.** *(**SUPERSEDED BY [P1135](p1135_agent_avatars_in_storage.md)** — the deploy this fact rests on is gone: the avatar is now a storage upload, not a committed-and-deployed static asset, so provisioning CAN complete inside one run. This is why `/points-publish` may now invoke `/provision-agent` inline at its halt point instead of only halting — see P1135 decision (c). The other two facts in this list — the mandatory sub-skill, and provisioning being needed outside this pipeline — still hold and still justify provisioning as a separate skill; only the deploy-boundary reason is gone.)* `gen-agent-avatar` Step 4 emits the avatar as a **static asset**, not a database value or an upload. So the sequence is: rights-cleared photo → generate → commit the asset → **deploy** → mint the `auth.users` row via the admin API → `create_or_reuse_agent_account` → verify. A filer that stops in the middle and waits for a deploy is already two skills; naming the boundary costs less than pretending it isn't there.
...
>
> **Static asset, not a storage upload — decided.** **SUPERSEDED BY [P1135](p1135_agent_avatars_in_storage.md)** — this whole paragraph is the rejection P1135 defeats (its Alternatives Considered entry below carries the reason). The avatar is now a storage upload; do not commit an avatar under `public/` per this paragraph. One path resolves on every host that serves the app (local, test, prod), so the avatar does not have to be uploaded twice into two different Supabase projects, and the likeness of a real person lands in version control where it can be reviewed and reverted. Cost: the asset must be deployed **before** the prod DB write, or the avatar 404s and the portrait channel silently drops to the initials fallback (`gravatar-avatar.tsx:134`). That ordering is a precondition below, asserted by HTTP status, not by memory.
