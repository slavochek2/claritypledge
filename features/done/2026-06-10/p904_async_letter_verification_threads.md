...
> **Downstream consumer (2026-06-13):** P904 is the async rail for **P918 Stage 2** (paired misunderstanding-risk diagnostic). In that flow both co-founders answer the diagnostic, a letter is generated from each other's responses, and they exchange + verify it *through P904* — turning P918's solo "felt gap on a stranger's letter" into their own measured gap, no synchronous /live required. P918 Stage 2 depends on P904 shipping. See `features/p918_misunderstanding_risk_self_diagnostic.md` (Reframe → Stage 2).
...
| R6 | **Point-level position-story slot is receiver-only.** The slot shows *only the receiver's* story on a point — never the sender's (the sender's reasoning is the letter body, reachable via the "Open story" ↗ link). Receiver with no story → "Add a story"; receiver with story → "View my story →"; sender → "View {receiver}'s story →" or nothing. | UX Design "Two affordances" table (point filled = "Jamie's story →") + R3 | **Bug found at UAT:** `get_letter_position_stories` returns rows for *both* participants, but the client folds them into a `Map<point_id, story>` (`letters-service.ts` `result.set(row.point_id, …)`) which **overwrites** — only one story per point survives. When the sender's row won, the receiver saw "View {sender}'s story →" and **lost the "Add a story" affordance entirely** (and a save would hit `story_points.UNIQUE(author_id, point_id)`). Fix: filter to receiver-authored rows **at iteration time** in `getLetterPositionStories` (skip `author_id === senderId`), so the receiver's row is never dropped. `[FOUNDER DECISION — made 2026-06-18]` |
| R7 | **Position-story view renders a proper story card, not raw text.** The `view` mode of `letter-position-story-dialog.tsx` shows author avatar + name + formatted (hashtag-stripped) body in card chrome, matching `live-story-card-expanded`, instead of a `whitespace-pre-wrap` text box. | UATer note ("why just text, not a proper story component") | A direct `getStoryWithPoints` re-fetch can't be used for the cross-party case — the position story is `visibility: 'private'`, so RLS returns `null` to the *sender* viewing the receiver's story. The card must render from data the `SECURITY DEFINER` RPC already hands over. Fix: extend `get_letter_position_stories` to also return `author_avatar_url`, `author_avatar_color`, `tags` (one migration to the existing RPC; `RETURNS TABLE` + `SELECT`). `[FOUNDER DECISION — made 2026-06-18]` |
