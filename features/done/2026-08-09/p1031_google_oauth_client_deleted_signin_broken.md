...
date_resolved: '2026-08-09'
root_cause: "OAuth client both Supabase projects pointed at was deleted from a co-owned legacy GCP project by another owner of that project"
...
**Closed 2026-08-09 with the two boxes above still open**, by founder decision. Left unticked deliberately: the outage (a critical prod auth failure) is fixed and evidenced, and neither open item bears on that. They are not "done" and must not be read as done — test has canary coverage but no human sign-in, and no console capture was taken during the prod flow.
