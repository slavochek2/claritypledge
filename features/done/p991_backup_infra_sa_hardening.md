...
- [x] The shared default service account no longer holds broad project-wide permissions — **evidence (via P998, 2026-07-17):** the broad role was removed after a fresh full-region enumeration confirmed no live consumer still ran as it (all migrated to dedicated identities; the one stopped VM was a recorded founder decision to leave). Pre-flight and read-back both verified with confirmed-success calls. The build path *did* depend on the broad role — the first post-removal deploy failed at the source-fetch step — and was restored with scoped, least-privilege grants that touch no backup bucket; a real deploy then succeeded. See private log 2026-07-17.
- [x] The shared account's broad grant on the second backup bucket is removed — **evidence (via P998, 2026-07-17):** removed *after* the broad role (the order this spec had inverted — the legacy ACL made it cosmetic beforehand). Denial proven by action, not by the binding's absence, via direct JSON API (gsutil is useless here — it dies at name expansion and never reaches the delete check): DELETE → 403 on the delete permission, GET of a real backup → 403, LIST → 403. Positive control: the same GET as an authorized identity → 200, and the target object survived the DELETE. The legacy project-editors→OWNER bypass is confirmed closed.
