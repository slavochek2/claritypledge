...
- [x] A real backup object from `gs://claritypledge-db-backups/` was selected using the documented marker rule, and its `sha256` verified against the marker — **evidence:** object `prod-backup-20260715-160327.sql.gz`; marker sha256 `b20f7f096213bd952df3ec7c20ef1d78ff4879cf01baf632193862a60991c639`; downloaded-file sha256 `b20f7f096213bd952df3ec7c20ef1d78ff4879cf01baf632193862a60991c639` — match. Note: **no marker existed at test start** — the P991 marker-write step merged 2026-07-15 13:26 UTC, after that day's only scheduled run (05:22 UTC). A backup was manually dispatched via `workflow_dispatch` (run `29430750200`, success, produced the marker) to get a testable object without editing `db-backup.yml`.
- [x] The dump restored into a scratch Postgres — **evidence:** restored into `postgres:17` via Docker (no local `psql`/`createdb` were installed on the test machine). 3346 lines of output; 826 role-does-not-exist errors, 129+128+80 pgvector-type errors, 16 `cron`/`net` schema-does-not-exist errors, 3 FK violations — full breakdown in `docs/technical/db-restore.md` "P997 restore findings". None of the errors touched `profiles`, `stories`, `points`, or `clarity_sessions`.
- [x] `profiles` row count in the restored DB matches `profiles_rows` in the marker — **evidence:** marker `profiles_rows=91`; restored DB `SELECT count(*) FROM public.profiles` → `91`. Match. Spot-checks: `stories`=20, `points`=39, `clarity_sessions`=238 (all non-zero, plausible).
- [x] Wall-clock restore time recorded — **evidence:** ~15 seconds for the documented procedure itself (download 6s + 4 integrity checks near-instant + `psql` load 1s + verification queries near-instant) once a verified backup exists and Docker/Postgres tooling is running. One-time environment setup (Docker Desktop cold start, `postgres:17` image pull) added ~2 min and won't recur on a machine with Docker already up.
- [x] `docs/technical/db-restore.md` corrected against what actually happened, and the "Open: prove the restore" section removed — **evidence:** replaced with "P997 restore findings (2026-07-15)"; status banner updated; added a Docker-based scratch-restore alternative to step 3.
- [x] Every divergence between the doc and reality is either fixed in the doc or filed as a follow-up spec — **no silent workarounds** — **evidence:** role/extension gaps and the manual-dispatch-after-pipeline-change gap are recorded as follow-ups in the doc's findings section, not fixed here (per Non-Goals).
- [x] The scratch DB and local dump are destroyed — **evidence:** `docker stop p997-scratch && docker rm p997-scratch`; downloaded `.sql.gz` and restore log removed from scratchpad. Confirmed via `docker ps -a` showing no `p997-scratch` container.
