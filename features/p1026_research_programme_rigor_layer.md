...
- **Rivals registry** — table: rival programme, its core claim, evidence for/against from our own data, progressivity status. Seed with four: (a) affect-first listening (Rogers/NVC — H-PopperianIncrement is the crucial discriminator experiment); (b) coordination/process tooling; (c) AI-mediated alignment (agent verifies/summarizes, no human protocol needed); (d) the null programme — affinity-based trust is adequate for most dyads (literature against it: closeness-communication bias; field data partially for it: 28 sessions with no cost named, the true-null demo). Implementation consequence: annotate H-PopperianIncrement in hypotheses.md as the rival-discriminator against rival (a) and move it to the front of the scientific test queue (the priority change itself `[FOUNDER DECISION]`).
