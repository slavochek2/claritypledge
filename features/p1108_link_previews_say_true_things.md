...
5. Write `src/tests/p1108-og-truthful-claims.test.ts` (all six bullet groups above, including the fetch-reject case and the (c)/(d) distinguishing pair). Run `npx vitest run src/tests/p1108-og-truthful-claims.test.ts` — CI-tier, no DB, no browser — and paste the passing output.
