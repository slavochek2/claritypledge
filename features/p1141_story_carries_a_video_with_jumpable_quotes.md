...
> **The trap, stated so nobody walks into it.** The cleaned transcript is what `/points-prepare`
...
- **2026-08-20** `docs/decisions.md:786-870` (not 2026-08-19 as this spec's Solution section states — a one-day citation slip worth a follow-up correction, not a blocker) is the actual entry recording the disguised-link finding: *"`linkify.ts:17` emits an anchor whose label is independent of its href, and `/points-prepare` harvests comment sections... XSS is blocked; phishing under a named person's account was not."*
...
**Decision 10 — The voice rules and the quotes-section label live in `/slava:content:points-prepare`, not `/points-publish`.**
`points-prepare` already owns every rule of this exact class — "never impute a position," "bald restatement," attribution-basis labelling per quote — and produces the actual story-draft text, including per-arguer quote selection. `points-publish` explicitly disclaims authorship ("This skill files; it does not author... wrong text ⟹ re-run prepare") and only enforces mechanical, uniform string requirements at filing time (the existing `#<event-tag>` presence check is the direct precedent). Full-name-not-pronouns and the `Supporting quotes from {Full Name}` label are both drafted narrative content, so they belong where narrative content is drafted. `points-publish` gains one new Quality Gate line asserting the label string is present verbatim in the read-back, mirroring how it already asserts the hashtag is present — a mechanical backstop, not a second place authoring the rule.
**Per-quote timecode extraction is also assigned to `points-prepare`:** it is the skill that already has each raw source video open for quote selection, and P1140's own guidance is explicit — resolve from the retained raw `.vtt`, never the `vtt-clean` ~30s output — which only the drafting step, not the filing step, has reason to touch.
...
- **The narrowing is supported by the 2026-08-20 finding's *scope*, but the finding does not support "narrow to bold/breaks/blockquotes/headings + link-match" as a *sufficient* fix — it only identifies the link-spoofing vector.** Verified the finding itself: `docs/decisions.md:837`, "A markdown link inside a quote publishes a disguised link under a real person's agent story. `linkify.ts:17` emits an anchor whose label is independent of its href, and `/points-prepare` harvests comment sections. At the gate the operator reads raw text; the viewer sees a clean link elsewhere. XSS is blocked; phishing under a named person's account was not." This is exactly what the spec says it is: one finding, about link labels, not about bold/headings/blockquotes, which the finding never mentions and never needed to reject. **The spec's characterization of the finding is accurate.**
- **What the relaxed renderer can newly emit that the flat ban prevented, beyond the link case already covered above:**
  ...
  - Both of the above are **markdown structure applied to content whose *source* is attacker-influenced** (harvested comments), which is the operative risk class throughout this review — the finding's own text names this precisely ("`/points-prepare` harvests comment sections") and the same provenance applies beyond just links.
...
3. Whether `/points-prepare`'s comment-harvesting logic already filters third-party (non-subject) identifiers before selecting quote/argument material — I did not read that skill file end-to-end.
...
9. **Markdown narrowing** — `linkify.ts` (Decision 8), including the label/destination match.
   ...
   **Decision 8's choice of `linkify.ts` over `marked`/`dangerouslySetInnerHTML` exceeds the
   ...
   to strip it.
10. **Skill updates** — `points-prepare.md` (voice rules, label, raw-`.vtt` timecode extraction — Decision 10), `points-publish.md` (Quality Gate line, row-shape update for `video_url`/`video_quotes`).
    **Open provenance question, not resolved by this spec** (security review, Data Protection):
    `/points-prepare` harvests YouTube comment sections, which are written by arbitrary private
...
- `.claude/commands/slava/content/points-prepare.md` — voice rules, quotes-section label, raw-`.vtt` timecode extraction
