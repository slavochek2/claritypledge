...
- [ ] Monitor prints spend against recorded budget for every live key
- [ ] Monitor emits a warning when spend exceeds recorded budget (cap-absent detection), proven by simulating that condition
- [ ] Monitor distinguishes "no billing data yet" from "zero spend" — proven with a key that has no export rows
- [ ] Monitor reports both drift directions (registry-without-project, project-without-registry)
...
- [ ] No secret, billing-account ID, or absolute user path committed to the public repo
