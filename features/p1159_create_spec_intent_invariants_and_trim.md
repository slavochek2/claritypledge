...
- [ ] **MISSED: file materially shorter (target ≤ ~330 lines).** `create-spec.md` is **560 lines**, up from 468. The ~180 lines came out as specced; the five additions the founder approved put ~250 back. Content was chosen over the number, deliberately and with the founder told. Reopen as its own trim item if the number matters more than the additions.
- [x] Remaining example carries frontmatter matching the Frontmatter section exactly
- [ ] A spec filed with the new skill on a **research-shaped** task carries pre-registered criteria — verified by filing one
- [ ] A spec filed on a **trivial** task carries no Invariants section and trips no gate — the anti-ceremony check
