...
- [ ] A grep for the prod master key across `.claude/commands/slava/` and `scripts/` returns only
      ...
      — *2026-09-15, one recorded exception:* 23 files remain; 18 are writes on the lock, 4 are
      warning/fixture text, and 1 archived prod-export script is a **read** behind the lock (no scoped
      credential serves a REST export that must bypass RLS). `/day-cp` names it 0 times. The canary's
      static scan finds 0 plaintext reads across scripts, skills and workflows (control 6/6, 0/3).
...
- [ ] `/day-cp` completes its user-activity block with the master key unset in the environment, and
      ...
      — *2026-09-15, half evidenced:* the block's seven queries ran on prod with the key unset, 7/7
      exit 0, all through the read-only helper. Denied set **not re-measured**: two new controls were
      blind (one failed on env-file lookup, one because the helper wraps every query in a SELECT, so any
      DDL is a syntax error). The no-fallback property is proven by P1214's canary (9/9); write denial
      rests on P1214's recorded SQLSTATE 25006.
- [ ] Each of the 28 retirement candidates carries a verdict (retired | live elsewhere) naming the
      ...
      — *2026-09-15:* 25 of 28 decided (8 live elsewhere, 17 retired), sample re-verified by command.
      3 undetermined: the two live surfaces that would decide them (hosting env, CI secret names) could
      not be enumerated this session. Table in `.private/docs/`.
...
- [ ] The stranded-signups CI summary reports the scoped read-only credential, and the master key is
      ...
      reference and the script's fallback are removed on this branch. Ticks when shipped.
