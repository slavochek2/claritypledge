...
- [x] AC-13: Surfaces not in scope are visually unchanged — `/stake/:tag`'s body, `/feed`, the
...
| AC-13 | **Pixel-identical before/after, same data.** The pre-P1323 base commit (`a6d71c798`, detached worktree) and this branch were served side by side against the SAME test database and captured at the same moment. Zero differing pixels at zero tolerance (Playwright's own comparator) for: `/stake/understanding` body and `/feed` body at 1280 and 375, and `/letter/st1` at 1280. **Control:** the `/stake` header region — which DID change — registered DIFFERENT, so "identical" is not a blind result. **Limit:** signed-out renders only |
...
| 34 | founder 2026-09-16, after seeing the desktop header | *"What do you think about instead of writing links in the big thing, making it a small link icon? Then everything shrinks and it's nice."* | **Built: the trigger is a 44x44 round link icon** (lucide `Link`), grey at rest, at every width, in the same slot between the primary CTA and the avatar. "Links" stays as the accessible name (sr-only) and the hover tooltip. Supersedes the outlined-navy boxed label P1179 put on the trigger; sheet rows keep that treatment | Mocked in the live signed-in header at 1280/375/320 BEFORE building, then built and re-shot. Position kept: a menu that OPENS belongs in the utility cluster, not the page-tab row. 44px tap target retained (P1179 AC-2 asserts it). **Known cost:** a chain icon can be read as "copy/share link"; the in-room instruction becomes "tap the link icon, top right". **Correction (adversarial review, Codex Sol):** the `title` tooltip appears only with a pointing device — on phones there is no visible "Links" text at all; the sr-only name serves assistive tech only |
| 35 | knock-on of row 34 | The icon is 25px narrower than the boxed label | "Start a Session" icon-only cutoff moved from 375px to **360px** — the label returns on 360px phones | Measured by the header e2e at 320/360/375 (no wrap, overlap or sideways scroll) and by eye at 360 |
| 36 | founder 2026-09-16: *"strange in /groups there is no links?"* | `/groups`, `/groups/:slug`, `/groups/:slug/join` were classified `public` — a mistake: "Groups" is a tab in the signed-in bottom nav | Reclassified `product`; `/groups` added to the AC-11d e2e | The remaining 29 `public` routes are marketing, legal, auth, dev-only or chrome-free; `/pledgers` is the one borderline case, left `public` |
| 37 | founder 2026-09-16: mobile menu *"too long and not scrollable"* (photo dated 2026-08-26) | The photo is the signed-out HAMBURGER menu, not the Links menu, and predates P1310's fix (committed 2026-09-14) | **No change.** Verified on production at 360x640 with a real CDP touch swipe: the menu scrolled 0 -> 267 (its maximum) and its last item "Create Account" ended fully on screen (616 of 640); control swipe on /manifesto scrolled the page 0 -> 350 | Emulated Android Chrome, not the founder's physical phone — a re-check on the device is the remaining step |
