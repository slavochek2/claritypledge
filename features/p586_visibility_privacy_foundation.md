...
1. **Run `/challenge-prd`** — stress-test Model B column-based approach, cross-visibility constraint edge cases, border consistency, privacy communication
2. **Run `/architect`** — `visibility` column migration, cross-visibility constraint design (CHECK vs RLS), enum removal, UPDATE policies, full `shared` grep, query-level exclusion from profile/feed
3. **Run `/ux`** — border color audit, privacy visual treatment, privacy communication on point cards, cross-visibility block UX
4. **Run `/generate-tests`** — RLS leak tests, cross-visibility constraint tests, visibility immutability tests, migration tests, display boundary tests
5. **Run `/spec-review`** — validate before implementation
6. **Run `/dev`** — implement (Phase 1: migration + column + constraints, Phase 2: UI cleanup + display boundaries)
