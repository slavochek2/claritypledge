...
2. **Back buttons inconsistent:** Some logged-in pages send users to landing page (`/`) instead of dashboard (`/home`). Settings goes to `/me`, but should go to `/home`. Events detail goes to `/events` list, but for logged-in users dashboard makes more sense.

3. **Login→Signup switch loses params:** `login-page.tsx:28` uses `window.location.href = '/signup'` which drops all URL params when switching.
...
| `src/app/pages/login-page.tsx` | Preserve params on login→signup switch |
...
| `src/app/pages/settings-page.tsx` | Change back button from `/me` to `/home` |
| `src/app/pages/profile-page.tsx` | Conditional back: `/home` if logged in, `/` if not |
...
- [ ] Task 4: Update `login-page.tsx` to preserve URL params when switching to signup
...
- [ ] Task 8: Update `profile-page.tsx` back button → conditional (logged in: `/home`, else: `/`)
