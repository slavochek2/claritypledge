...
**Complication:** The verified-understanding model (decisions.md 2026-05-31 [product]) upgrades the agreement to a number-first commitment that carries the codified algorithm. But the *number + min* mechanic is only coherent **bilaterally** — which is exactly where the agreement lives (two opted-in people), unlike the unilateral pledge. The agreement is also the **test population** for the falsify (pairs, per P853). So the agreement, not the pledge, is the first place the new model should ship — and it can't, because there is no versioning to ship it behind or to roll back to.
...
**Stage B — number-first v-next + addendum.** Add the new version (shared core + the bilateral addendum that codifies the algorithm) and update `/partner-template`. The agreement carries the **codified algorithm** ("tested by explain-back"):

> Our numbers begin as honest assumptions. Until one of us explains back, the lower number is the most we can claim we share. When the lower number matters, the listener explains back, without judging the idea — that is the test. The speaker, who owns the intention, shows what landed and what didn't, and the listener revises, up or down, never above what they honestly find. If the explanation doesn't land, we go again. What we can finally claim as verified is the lower of our honest numbers after this test, and we both accept it can still miss.

(Addendum wording is a `[FOUNDER DECISION]` — draft to react to, not final. Pledge-vs-agreement split: the pledge *accepts* the standard + gives a number; the agreement *enacts* the algorithm — the min as a joint action lives here, where it is coherent.)
...
- **ACCEPT — Addendum verbosity.** The codified algorithm is longer than a one-line oath; the agreement is the right home for it (bilateral practice contract), so length is tolerable.
...
- Do NOT finalize the addendum wording without founder sign-off + `/challenge-prd`.
...
- [ ] Addendum wording passed `/challenge-prd` + founder sign-off
