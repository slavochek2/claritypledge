...
- **Meta (separate, not fixed here):** `pre-commit-checks.sh:74` runs bare `npx tsc --noEmit`, which against the root `tsconfig.json` (`files: []`, project references) checks nothing — a no-op type gate. The real app typecheck (`tsc -p tsconfig.app.json`) reports ~1117 errors. That is how this undeclared identifier shipped. Filed as **P861** — make pre-commit run `tsc -b` / `-p tsconfig.app.json` (needs a cleanup strategy for the ~1117 pre-existing errors).
