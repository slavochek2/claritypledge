...
- The resume path MUST survive a kill in the commit→journal-write window without looping silently: a cherry-pick that committed but whose `landed_sha` was never recorded is detected on the next `--resume` (immediate child of the recorded `pre_pick_head` carrying the source's author identity) and surfaced via a refuse-and-diagnose, NOT auto-recorded. Author identity (email + author-date + subject) is preserved by cherry-pick but is forgeable by any same-source pick (a co-tenant cherry-pick after `--skip` is indistinguishable), so it must never by itself drive a journal write or branch teardown — that would risk silent loss of a pending commit from main. Convergence is an explicit operator step (`--mark-landed`, which validates the sha is on main) after the operator verifies the candidate carries the change.
