...
- [x] `/` hero reads "Keep the hire you can't / afford to lose." + a single "Book your free alignment audit" CTA → `/intro` — **wedge word resolved this session** ("de-risk" rejected, see Resolved Decisions #9); live-session microcopy (`AUDIT_MICROCOPY` = "A live 1:1 session. We find the blind spot in how you align with your team. Starts with a 15-min call.") rendered under BOTH the hero CTA and the closing CTA; hero sub reads "Make misalignment easy to reveal and safe to bridge."; no coach/price/co-founder content on the hero otherwise. **Corrected 2026-07-15:** an intermediate shortening pass dropped the "A live 1:1 session" clause from visible copy, leaving the promise alive only in the SEO `description` meta tag — disclosure a reader never sees. Restored to copy and pinned by an exact-match e2e assertion whose failure path was exercised (drop the clause → `Expected: 2, Received: 0`).
...
- [x] Agreement section heading reads "Protect the relationship before your interests quietly diverge." — resolved this session, replacing the leftover "Protect the hire so it survives early disagreements" placeholder. **Corrected 2026-07-15:** this line previously claimed "interests **and values** quietly diverge" while the code said "interests quietly diverge" (`program-page.tsx`, `title` prop of the agreement section). The code is what the founder reviewed; the spec text was the stale half. Singular "the relationship" is deliberate — it points at this hire, not at relationships generically.
...
- [x] `TemplateStamp` — added a `sr-only` span ("Template — sample agreement, not a real signed document") alongside the existing `aria-hidden` decorative stamp, so screen-reader users get the mock-document disclosure. **Corrected 2026-07-15:** this line originally justified that span as giving assistive tech "the disclosure the visual stamp already gives sighted users" — a false premise. The visual stamp gave sighted users on **phones** nothing: its `IntersectionObserver` used `threshold: 0.6`, a ratio *of the observed element*, while the stamp renders ~867px wide at its pre-landing `scale(2.7)`. At 375px the greatest achievable ratio is 0.43 (measured empirically at 0.433), so the callback never fired and the watermark sat at `opacity: 0` on every viewport under ~520px — a deadlock, since the stamp had to fire to shrink to the 336px that would have satisfied the threshold. Desktop reached 1.0, which hid it. The demo agreement (Einstein/Teresa "signing") therefore shipped to phones with **no visual sample marker** on `/`, `/coach` (live), `/founder` and `/partner-template`. Trigger is now viewport-relative (`threshold: 0` + `rootMargin: "-25% 0px"`), so the element's own size cannot defeat it. Verified live: `opacity 0 → 1` at 375 on `/` and at 320 on `/coach`.
- [x] Visual-QA pass — **cleared 2026-07-15** by a blind subagent (screenshots + checklist only, no diff, no intent) across `/`, `/founder`, `/intro` at 375/320/desktop. No CRITICAL survived verification. It found **three real, pre-existing, live-in-prod bugs** that desktop-width checking had hidden — all now fixed with regression tests:
  - **`/intro` was unbookable on a phone** (`43da4052`) — the destination of the site's primary CTA showed only struck-through past dates. One flat `minHeight: 580px` for all widths, but Google stacks the picker vertically below ~520px and needs ~880px there.
  - **The TEMPLATE watermark never rendered below ~520px** (`30a9a62c`) — `IntersectionObserver` `threshold: 0.6` is a ratio *of the element*, and the stamp is ~867px wide at its pre-landing `scale(2.7)`; max achievable ratio at 375px is 0.43. A deadlock: it had to fire to shrink to a size that could fire. The mock agreement shipped to phones with no "sample" marker.
  - **The pledger `+N` badge** was both wrong (`b8e8a2a2` — subtracted the row limit, not the avatars drawn: 6 + 683 ≠ 691) and unreadable (`14202b1d` — painted under the last avatar, so it read "685").
  - **Refuted, do not re-file:** identical "PS" avatar initials are **test seed data** — the dev server points at the test project; prod returns 12 distinct initials, 4 with photos. The `/intro` "blurred band" is the sticky nav's `backdrop-blur`. The desktop hamburger is the intentional P856 dropdown.
  - **Open MEDIUM/LOW, non-blocking:** `/founder` testimonial cards ~3× height mismatch (placeholder avatar beside a real photo); footer container wider than page content and left-unbalanced; 320 eyebrow pill's icon orphaned once the label wraps; platform-card body copy starting at uneven heights; thought-bubble connector dots off-centre and very low contrast; nav CTA says "Book alignment audit" while the hero says "Book your **free** alignment audit".
  - **Method note (load-bearing):** the first QA round returned two CRITICALs that were both artifacts of the *capture*, not the page — a single `scrollTo(0, scrollHeight)` jump never intersects the middle sections, so their `once: true` reveals never fire and they photograph blank; and `fullPage` capture blanks cross-origin iframes. Captures must scroll stepwise (~300px) and verify zero low-opacity content before shooting; `/intro` must be captured per-viewport, never full-page.
