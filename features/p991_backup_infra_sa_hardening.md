...
2. Investigate the file-upload service's exact permission needs by reading its source and current deployment config — don't assume from the service name. Create a dedicated service account with only those permissions. Redeploy (zero-downtime for this consumer) and verify a real end-to-end upload still works.
   - **2a. Same deploy, do it here not separately:** this service holds a shared secret as a literal env var rather than a managed-secret reference. Move it to a managed secret reference and rotate the value **as part of this step's redeploy**. Sequencing is the whole point: a managed-secret reference requires granting accessor permission to the runtime service account, so doing it before this migration grants it to the shared account being de-privileged, then re-grants to the new one — inside this step it's one grant to the right identity, and one redeploy instead of two. **The secret is shared with an upstream proxy that calls this service, and the check is a single-value equality with no dual-accept — so a naive rotation has a hard downtime window.** Rotation traps (upstream instance caching, ordering, stale local copies) are enumerated in the private infra decisions log; read that entry before starting, not mid-cutover.
...
- **The secret rotation in 2a has a hard cutover window** — single-value equality check, no dual-accept, so whichever side flips first the other is rejected until it catches up. The client's retry logic has a short timeout and finite backoff; it will NOT cover a multi-minute window. Mitigation: rotate when no live session is in progress, or add temporary dual-accept (two deploys, zero downtime). Verify the upstream proxy actually picked up the new value — a warm instance may hold the old one until it recycles.
...
- [ ] That service's shared secret is a managed-secret reference (not a literal env var) and the value has been rotated; a real end-to-end upload still works after the rotation, and the stale local copy is updated
...
- [ ] The backup workflow's integrity check catches a truncated-but-nontrivial file, not just a near-empty one
- [ ] The backup workflow has a concurrency guard preventing overlapping runs
