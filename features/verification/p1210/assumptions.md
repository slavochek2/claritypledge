...
| # | Call | Chosen over | What would reverse it |
|---|---|---|---|
| A-11 | The DW-1 table's *Stage output* column names tokens that ACTUALLY OCCUR in the owner files — `phase_0_verdict`, `SOURCE-FIDELITY`, `Gate 2`, `Story Drafts`, `4b-iii`, `is_synthesized`, `Print the room back`, `12 min per point` | The tokens the spec's own §Problem table used (`phase_0_note`, `loss_currency`, `room_key`, `arbiter_tag`) | Those four were grepped for and occur **zero** times in any pipeline file — they are run-file schema keys, not stage-file tokens, so a table built on them would have failed DW-1's own resolver. The spec's table is prose about the objective; the doc's table has to resolve |
| A-12 | Rows 8 and 9 have owner `events/clarity-practice-event.md` rather than the spec's "the event" | Leaving the owner as an abstract actor | DW-1 requires every owner to resolve to an existing pipeline file. The event doc is the file that owns those two conditions, and their *Stage output* stays `—` because only an event can settle them |
| A-13 | Condition 7's evaluability was made concrete by writing the 12-min-per-point rule into `prepare.md` | Pointing condition 7 at the event doc | RD-4 decided the budget so that condition 7 becomes evaluable *by the pipeline*; a condition owned by a stage needs its rule stated in that stage's file, or the resolver has nothing to find |
| A-14 | `publish.md`'s story-length assert moved from 1,500 to the 10,000 DB limit | Leaving publish asserting 1,500 while `story-draft` states "no character ceiling" (§7) | §7 withdrew the build-time ceiling. Leaving the downstream assert would have blocked every run under the new unit — an incoherent pipeline, not a conservative one. Reverse it by restoring the ceiling in `docs/story-craft.md` §1 first; `publish` follows the drafting brief, it does not set it |
| A-15 | `two-callers.mjs` exempts six HARNESS modules by an explicit named allowlist, asserted in the test | Requiring every `.mjs` under `scripts/points/` to be invoked by a skill file | The sweep, the vacuity scan, this check, the fixture verifier and the two fixture builders are called by the verification contract and CI, not by a pipeline stage. Requiring a stage to invoke them would put verification plumbing into the run-of-show. The allowlist is asserted by name so adding to it is a visible edit, never a silent widening |
| A-16 | DW-23's forbidden-token list is committed as **sha256 digests**, generated by `redact-run.mjs --emit-forbidden` | Committing the surnames and sentences the scan looks for | A plaintext list would put the names into the public file the scan exists to keep them out of. The digests let half (a) run in CI with no run file and no name in the repo |
| A-17 | The must-fail fixtures for `rule-present.mjs` are **generated** by `build-rule-fixtures.mjs` from the real files, not hand-written | Hand-writing five stripped copies | A hand-written fixture drifts from the sentence it is supposed to be missing. **And the first generated version was BLIND**: its header named the deleted rule, the predicate matched its own strip note, and all five must-fail fixtures came back RESOLVE while looking green. Caught by running the controls and expecting REJECT — not by reading the code. The header no longer names the rule |
| A-18 | Rule regexes are anchored on each file's own wording, one rule per LINE | Paraphrase-tolerant patterns spanning lines | A `[^.\n]*` pattern silently fails across a line break, and two rules sharing one line make a must-fail fixture unable to delete one without the other. One rule, one line, is what makes the stripped fixture name exactly the missing rule |
