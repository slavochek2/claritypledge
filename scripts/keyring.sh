...
#      enroll-from a temp file filled from the password-manager copy — see
#      docs/technical/credential-keyring.md § Recovery, P1322.)
...
FATAL: could not read the critical credential ${key}.
  ...
  If lost:   re-enroll from the password-manager copy (credential-keyring.md § Recovery)
...
_keyring_cmd_enroll() {
  ...
  echo "from the password-manager copy instead: see credential-keyring.md, Recovery section"
  echo "Enrolled a new or changed value? Update the password-manager copy the same day."
  return $rc
}
