...
# -----------------------------------------------------------------------------
# Z2. Co-located spec auto-close (P800): branch DELIVERS (edits) a spec for a
#     second P-number (p121) that already existed on main — ship p120 must
#     auto-close p121 alongside. p121 is seeded on main and edited on the
#     branch (not created there) so this canary tests the "co-delivered"
#     case its name claims — a branch-created-only spec is Z3's job (P1105:
#     the original fixture created p121 only on the branch, which is
#     actually the bug shape, not the co-delivered shape; split per the
#     reproduce commit note "Z2 needs splitting, not just 'still passes'").
# -----------------------------------------------------------------------------

# p120 (primary) and p121 (co-located) specs both committed to main BEFORE
# the branch exists — resolve_ship_spec finds p120 before Phase 1; p121 must
# already be on main for this canary to test "edited", not "created".
...
# Feature branch: source commit + p121 spec EDIT (delivery, not filing).
# p121 already exists on main; Phase 1 cherry-picks bring the edit to main so
...
# before the branch existed — must NOT be auto-closed by Phase 2b. Being
# created or edited on the branch is not delivery; detect_cospecs() currently
# has no add-exclusion, so it cannot tell "co-delivered" from "merely filed"
# and Phase 2b closes both. This canary is deliberately shaped like Z2 (same
# primary+co-located structure) with the assertion inverted, because Z2's own
# p121 fixture IS the P1105 bug shape — Z2 pins today's (wrong) behavior for
# a branch-only-created spec, not the "edited on main first" case its own
# name implies. /fix must resolve that overlap (see P1105 Fix Approach note).
# -----------------------------------------------------------------------------

cat > "$SCRATCH/main/features/p122_primary.md" <<'EOF'
---
status: qa
type: task
rank: 1
...
( cd "$SCRATCH/main" && git checkout -q -b feature/p122-demo ) >/dev/null
echo "fix_a" > "$SCRATCH/main/p122_fix.txt"
( cd "$SCRATCH/main" && GIT_AUTHOR_DATE="2024-01-01T10:00:00" GIT_COMMITTER_DATE="2024-01-01T10:00:00" \
  git add p122_fix.txt && git commit -qm "p122: fix a" ) >/dev/null
# p123 is FILED mid-branch — a follow-up spec, never on main. status: backlog,
# matching the P1105 reproduction steps exactly.
...
Z3_OUT="$(cd "$SCRATCH/main" && capture_r bash "$GIT_OPS" ship p122)"

# Primary still closes normally — this canary isolates the add-exclusion gap,
# it does not claim the whole close path is broken.
if [[ ! -f "$SCRATCH/main/features/done/2026-04-22/p122_primary.md" ]]; then
  echo "$Z3_OUT" >&2
  fail "Z3: p122 spec not moved to done/ — primary spec close failed (unrelated to the P1105 gap this canary targets)"
fi
# The bug: p123 was only ever FILED on the branch, never delivered — it must
# stay in features/ at its original status, not be auto-closed.
if [[ ! -f "$SCRATCH/main/features/p123_followup.md" ]]; then
  echo "$Z3_OUT" >&2
  fail "Z3 (P1105): p123 was filed on the branch and never implemented, but ship auto-closed it anyway — it is missing from features/"
fi
if [[ -f "$SCRATCH/main/features/done/2026-04-22/p123_followup.md" ]]; then
  echo "$Z3_OUT" >&2
  fail "Z3 (P1105): p123 — filed mid-branch, never implemented — was moved to features/done/. A spec may only be auto-closed by a ship that DELIVERED it; being created on the branch is not delivery."
fi
if ! grep -q '^status: backlog$' "$SCRATCH/main/features/p123_followup.md" 2>/dev/null; then
  echo "$Z3_OUT" >&2
  fail "Z3 (P1105): p123 status was rewritten — it should remain 'backlog', untouched, since it was never delivered"
fi
pass "Z3 (P1105): spec filed mid-branch (never on main) is left untouched by ship — not auto-closed as co-located"
...
# Primary spec on main, PLUS the co-located p165 spec (frontmatter MISSING)
# already on main too — it must be DELIVERED (edited on the branch), not
# merely filed there, or P1105's add-exclusion correctly removes it from
# Phase 2b's close attempt before the frontmatter-rewrite-failure path this
# canary targets ever runs.
...
# Branch: one source commit + an edit to the co-located spec whose
# frontmatter is (still) MISSING. A spec split out earlier, then touched
# again mid-branch, is exactly how this arises.
...
echo "Still no frontmatter after this edit." >> "$SCRATCH/main/features/p165_broken.md"
( cd "$SCRATCH/main" && GIT_AUTHOR_DATE="2024-01-01T10:01:00" GIT_COMMITTER_DATE="2024-01-01T10:01:00" \
  git add features/p165_broken.md && git commit -qm "p165: edit" ) >/dev/null
( cd "$SCRATCH/main" && git checkout -q main ) >/dev/null
