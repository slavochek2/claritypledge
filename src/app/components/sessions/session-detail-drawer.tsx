...
function RoundRow({ item, index }: { item: SessionHistoryItem; index: number }) {
    ...
    <Drawer open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DrawerContent className="max-h-[85vh] pb-safe">
        <DrawerHeader className="relative">
          ...
          <DrawerDescription>
            {session ? `${session.roundCount} round${session.roundCount !== 1 ? 's' : ''} completed` : ''}
          </DrawerDescription>
        </DrawerHeader>
            ...
            {session.sessionHistory.length === 0 ? (
              ...
              <ul aria-live="polite" aria-label="Session detail loaded">
                {session.sessionHistory.map((item, i) => (
                  <RoundRow key={i} item={item} index={i} />
                ))}
              </ul>
            )}
          </div>
