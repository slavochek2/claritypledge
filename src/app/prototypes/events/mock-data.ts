...
Participants learned practical techniques for clearer conversations.`,
  ...
  // Event cancelled by another host (Maya) - I was attending
  {
    ...
    description: `A hands-on workshop exploring effective communication techniques for difficult conversations.

**What we'll cover:**
- Active listening fundamentals
- Reframing disagreements constructively
- Managing emotional triggers
- Practice scenarios with feedback

**Why this was cancelled:**
Unfortunately, Maya had a family emergency and couldn't host this session. We hope to reschedule soon!`,
  ...
  // Event I cancelled as host
  {
    ...
    description: `A casual lunch session where we explore real-world communication challenges.

**Agenda:**
- 12:00 - Lunch arrives (provided!)
- 12:15 - Topic introduction: "Giving Feedback Without Triggering Defensiveness"
- 12:30 - Group discussion and examples
- 12:50 - Key takeaways

**Why this was cancelled:**
Had to reschedule due to a conflict with an investor meeting. Will announce new date soon!`,
...
export function getUpcomingEvents(): MockEvent[] {
  ...
  return mockEvents
    .filter(e => {
      ...
      // Include upcoming OR cancelled events scheduled for the future
      return (e.status === 'upcoming' || e.status === 'cancelled') && eventDate > now;
    })
...
export function getPastEvents(): MockEvent[] {
  ...
  return mockEvents
    .filter(e => {
      const eventDate = new Date(e.datetime);
      // Include completed OR cancelled events scheduled for the past
      return e.status === 'completed' || (e.status === 'cancelled' && eventDate <= now);
    })
