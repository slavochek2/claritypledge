      ...
      try {
        ...
        if (userProfile) {
          ...
          if (!userProfile.isVerified) {
            const { error } = await import("@/polymet/data/api").then(m => m.verifyProfile(id));
            if (error) {
          ...
          // Redirect to profile (slug-based) with firstTime param after 3 seconds
          const slug = userProfile.slug || id;
          setTimeout(() => {
      ...
      <div className="text-center space-y-6 max-w-md">
        ...
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Redirecting to your pledge...
          </p>
          <Link to={`/p/${profile?.slug || id}?firstTime=true`}>
            <Button
              size="lg"
              className="bg-blue-500 hover:bg-blue-600 text-white"
            >
