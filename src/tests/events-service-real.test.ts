...
describe('realEventsService', () => {
  ...
  describe('getUserPastEvents', () => {
    const mockPastEvent = {
      ...
      datetime: '2025-12-01T18:00:00Z', // Past date
      duration_minutes: 120,
      timezone: 'America/Los_Angeles',
      location: 'Test Location',
      host_id: 'user-1',
    ...
    it('returns past events user hosted', async () => {
      // First call: RSVP query (no RSVPs)
      mockSelect.mockReturnValueOnce({
        ...
        lt: vi.fn().mockReturnValue({
          or: vi.fn().mockReturnValue({
            order: vi.fn().mockReturnValue({
              limit: vi.fn().mockResolvedValue({ data: [mockPastEvent], error: null }),
            }),
          }),
        }),
      });

      const events = await realEventsService.getUserPastEvents('user-1');
    ...
    it('returns past events user attended via RSVP', async () => {
      const attendedEvent = {
        ...mockPastEvent,
        id: 'evt-past-2',
        host_id: 'other-user', // User is not the host
        host: {
          ...mockPastEvent.host,
          id: 'other-user',
      ...
      // First call: RSVP query (user RSVP'd to evt-past-2)
      mockSelect.mockReturnValueOnce({
        eq: vi.fn().mockResolvedValue({ data: [{ event_id: 'evt-past-2' }], error: null }),
      });
      ...
      mockSelect.mockReturnValueOnce({
        lt: vi.fn().mockReturnValue({
          or: vi.fn().mockReturnValue({
            order: vi.fn().mockReturnValue({
              limit: vi.fn().mockResolvedValue({ data: [attendedEvent], error: null }),
            }),
          }),
        }),
      });
      ...
      expect(events.length).toBe(1);
      expect(events[0].hostId).toBe('other-user'); // Confirms user attended, not hosted
    });

    it('returns empty array on error', async () => {
      // First call: RSVP query succeeds
      mockSelect.mockReturnValueOnce({
        eq: vi.fn().mockResolvedValue({ data: [], error: null }),
      });

      // Second call: Events query fails
      mockSelect.mockReturnValueOnce({
        lt: vi.fn().mockReturnValue({
          or: vi.fn().mockReturnValue({
            order: vi.fn().mockReturnValue({
    ...
    it('handles RSVP query error gracefully', async () => {
      // First call: RSVP query fails
      mockSelect.mockReturnValueOnce({
        eq: vi.fn().mockResolvedValue({ data: null, error: { message: 'RSVP error' } }),
      });

      // Second call: Events query (still runs with empty RSVP list)
      mockSelect.mockReturnValueOnce({
        lt: vi.fn().mockReturnValue({
          or: vi.fn().mockReturnValue({
            order: vi.fn().mockReturnValue({
      ...
      // Should still return hosted events even if RSVP query failed
      expect(events.length).toBe(1);
    });

    it('returns events sorted by most recent first', async () => {
      const olderEvent = { ...mockPastEvent, id: 'evt-old', datetime: '2025-01-01T18:00:00Z' };
      const newerEvent = { ...mockPastEvent, id: 'evt-new', datetime: '2025-11-01T18:00:00Z' };

      // First call: RSVP query
      mockSelect.mockReturnValueOnce({
        eq: vi.fn().mockResolvedValue({ data: [], error: null }),
      });

      // Second call: Events query - order by datetime desc
      mockSelect.mockReturnValueOnce({
        lt: vi.fn().mockReturnValue({
          or: vi.fn().mockReturnValue({
            order: vi.fn().mockReturnValue({
              limit: vi.fn().mockResolvedValue({ data: [newerEvent, olderEvent], error: null }),
            }),
          }),
        }),
      });
      ...
      expect(events[0].id).toBe('evt-new'); // Newer event first
