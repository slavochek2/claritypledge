...
describe('getViewState — P626: listener stays idle until speaker submits', () => {
  it('listener stays on idle when speaker clicked Speak (ratingInitiatedBy set, no submission)', () => {
    // P626 Bug 1: Before the fix, ratingInitiatedBy was set on click, but the
    // listener should stay on idle until the speaker actually submits.
    // getViewState does NOT check ratingInitiatedBy — it only sees ratingPhase + submissions.
    // With the fix, ratingInitiatedBy is only set on submit (same time as ratingPhase: waiting).
    expect(getViewState(input({
      ratingPhase: 'idle',
      isLocallyRating: false, // listener is NOT locally rating
      myRatingSubmitted: undefined,
      partnerRatingSubmitted: undefined,
    }))).toEqual({ view: 'idle' });
  });

  it('listener enters responder-drawer only after speaker submits (submission mismatch)', () => {
    // P626: The correct transition — listener sees the drawer only when submission mismatch occurs
    expect(getViewState(input({
      ratingPhase: 'idle', // May still be idle due to Realtime delay
      isLocallyRating: false,
      myRatingSubmitted: undefined,
      partnerRatingSubmitted: true, // Speaker submitted
    }))).toEqual({ view: 'responder-drawer' });
  });

  it('speaker goes to local-rating immediately on Speak click', () => {
    // P626: Speaker's own view should change instantly (local state)
    expect(getViewState(input({
      isLocallyRating: true,
      ratingPhase: 'idle',
    }))).toEqual({ view: 'local-rating', showDrawer: false });
  ...
  it('mode switcher should not appear when ratingPhase is not idle', () => {
    // P626 Bug 3: mode switcher visibility is handled in JSX (not getViewState),
    // but we verify that getViewState returns non-idle for the listener after submission
    expect(getViewState(input({
      ratingPhase: 'waiting',
      isChecker: false,
      myRatingSubmitted: undefined,
