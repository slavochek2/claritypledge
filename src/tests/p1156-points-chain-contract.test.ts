 ...
 * schema home; and points-publish repointed by skill name, not stage number.
 *
...
const SELECT = read('.claude/commands/slava/content/points-select.md');
...
describe('p1156 — the untrusted-input rule lives in EVERY chain skill, in full', () => {
  it.each([
    ...
    ['points-prepare', PREPARE],
    ['positions-create', POSITIONS],
    ['story-create', STORY],
    ['points-publish', PUBLISH],
...
describe('p1156 — the split is relocation, not revision', () => {
  ...
  it('stages 6 and 8 are tombstoned to their new homes, not silently deleted', () => {
    ...
    expect(PREPARE).toMatch(/positions-create/);
    expect(PREPARE).toMatch(/story-create/);
  });

  it('positions-create selects quotes BEFORE positions and keeps the hypothesis warning', () => {
  ...
  it('story-create enforces the build-time limits the spec requires', () => {
...
describe('p1156 — publish is repointed by skill name, not stage number', () => {
  it('the two STOP conditions resolve to skills that actually emit the field', () => {
    expect(PUBLISH).toContain('/slava:content:points-select');
    expect(PUBLISH).toContain('/slava:content:positions-create');
    expect(PUBLISH).toContain('/slava:content:story-create');
  });
  ...
  it('no reference to prepare stage numbers survives in publish', () => {
    ...
    expect(PUBLISH).not.toMatch(/points-prepare` Stage/);
