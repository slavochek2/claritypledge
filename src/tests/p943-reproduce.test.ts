...
describe('P943: DST-aware UTC for webinar series seeding', () => {
  ...
  // RED before fix: naive stub returns 08:30Z (UTC-2) instead of 09:30Z (UTC-1)
  it.fails('winter (2026-10-29, CET): 10:30 Berlin → 09:30:00Z (not 08:30:00Z)', () => {
    expect(wallClockToUTC(BERLIN, 2026, 10, 29, 10, 30)).toBe('2026-10-29T09:30:00.000Z');
  });

  // RED before fix: clocks go back at 03:00 on 2026-10-25; 10:30 is already CET
  it.fails('DST boundary day (2026-10-25): 10:30 Berlin → 09:30:00Z', () => {
