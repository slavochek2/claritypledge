...
describe('P986: generateSlug non-ASCII title', () => {
  it('produces a readable, non-empty title portion for a fully non-Latin title', () => {
    const slug = generateSlug('这是一个活动');
    const titlePortion = slug.split(/-\d{4}-\d{2}-\d{2}-/)[0];
    expect(titlePortion).not.toBe('');
    expect(slug.startsWith('-')).toBe(false);
  });

  it('leaves ASCII titles unchanged (no regression)', () => {
    const slug = generateSlug('Community Meetup');
