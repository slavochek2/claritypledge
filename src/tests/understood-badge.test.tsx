...
describe('UnderstoodBadge', () => {
  it('renders with count=0 and shows "0 understood"', () => {
    render(<UnderstoodBadge count={0} />);
    expect(screen.getByText('0 understood')).toBeTruthy();
  });

  it('renders with positive count', () => {
    render(<UnderstoodBadge count={3} />);
    expect(screen.getByText('3 understood')).toBeTruthy();
  });
  ...
  it('applies size="xs" variant', () => {
    ...
    const badge = screen.getByText('1 understood');
  ...
  it('applies custom className', () => {
    ...
    const badge = screen.getByText('0 understood');
    expect(badge.className).toContain('mt-4');
  });

  it('handles large counts', () => {
    render(<UnderstoodBadge count={999} />);
    expect(screen.getByText('999 understood')).toBeTruthy();
  });
});
