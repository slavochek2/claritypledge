-- P681: Add per-point visibility to seal_and_send_letter RPC.
--
-- The seal RPC snapshots story content into letter_story_snapshots.point_config
-- but didn't include per-point visibility. Result: sealed letters lost track of
-- which points were private vs public, and the read-time mapper defaulted
-- everything to 'public'.
--
-- This migration adds 'visibility' to the per-point jsonb_build_object.
-- No backfill needed — letter-snapshot-mapper.ts uses a fallback chain:
-- point.visibility ?? snapshot.visibility ?? 'public'
