-- P683 bug fix: restore engagement RPC access after TOS consent.
--
-- Root cause: commit c77be9bd added token-replay protection to create-and-open-letter
-- by setting invitation_expires_at = now() the moment a delivery is linked to a
-- receiver. This correctly prevents replaying the token to mint fresh auth sessions.
-- However, the four P642 engagement RPCs validated the SAME column with:
--   AND (ld.invitation_expires_at IS NULL OR ld.invitation_expires_at > now())
-- That predicate is false immediately after TOS consent, so every engagement call
-- (submit_rating, submit_point_response, reveal_prediction, update_delivery_status)
-- fails with the RPC returning false/"Invalid or expired token" for the rest of the
-- reader session.
--
-- Fix: drop the expiry predicate from all four engagement RPCs. The session-minting
-- replay defense (in create-and-open-letter) is unaffected — invitation_expires_at
-- continues to gate the generateLink call. Engagement writes are already idempotent
-- (ON CONFLICT DO NOTHING) and scoped to sealed letters (cl.status = 'sealed').
--
-- Reuse: bodies are verbatim clones of 20260404110807_p642_anon_letter_engagement_rpcs.sql
-- minus the expiry predicate line. No logic or return-type changes.
...
BEGIN
  ...
  -- not engagement; see migration header for full explanation)
