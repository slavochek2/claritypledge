-- P699: Auto-resolve best delivery for sender when p_delivery_id IS NULL.
--
-- Previous version left v_actual_delivery_id = NULL for sender with no delivery_id,
-- causing ratings=[] and gap=undefined on the sender results page.
-- Now auto-picks the most relevant named delivery (completed > in_progress > opened > sent).
...
BEGIN
  ...
  IF auth.uid() = v_sender_id THEN
    ...
    IF p_delivery_id IS NOT NULL THEN
      -- Explicit delivery supplied — resolve that specific delivery
    ...
    ELSE
      -- Auto-resolve: pick the most relevant named delivery
      -- Priority: completed → in_progress → opened → sent; tie-break: newest first
      SELECT ld.receiver_profile_id, ld.id
      INTO v_receiver_profile, v_actual_delivery_id
      FROM letter_deliveries ld
  ...
  -- Populated for receiver, or for sender when delivery_id is resolved
