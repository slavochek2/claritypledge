-- Migration: P699 fix — extend get_letter_results to return full profile objects
-- Replaces sender_name TEXT / receiver_name TEXT with sender_profile JSONB / receiver_profile JSONB.
-- Only one caller (getLetterResults in letters-service.ts) — no backward compat needed.
-- Profile JSONB contains: name, avatar_url, avatar_color, role, has_pledged, ears_count.
-- DROP required: cannot change return type of existing function without re-creating.
