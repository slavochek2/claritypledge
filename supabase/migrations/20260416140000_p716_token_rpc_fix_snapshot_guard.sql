-- P716 patch 2: restore point_config snapshot guard regressed by 20260416130000.
--
-- 20260416130000 used 20260414000000 (P705 initial) as its baseline, not
-- 20260415143000 (P705 fix), which had deliberately replaced the story_points
-- JOIN with jsonb_array_elements(point_config->'points').
--
-- Root cause of that earlier fix: letters are immutable snapshots. A point may
-- exist in point_config (sealed at send time) but be absent from story_points
-- (deleted from the story after seal, or test fixture without story_points row).
-- The story_points JOIN returned false for those legitimate points, blocking
-- submission with no visible error.
--
-- This migration is the canonical, complete version of submit_point_response_by_token:
--   - All P705 guards (NULL, enum, anon-auth, authorization scope via snapshot)
--   - P716 dual-write change: auth.uid() instead of v_receiver_id + is_verified,
--     so the write succeeds even when receiver_profile_id is still NULL.
