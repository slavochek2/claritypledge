-- P755: Restore get_inbox_items() in-progress visibility + progress fields.
--
-- P725 used CREATE OR REPLACE to add actor_slug but silently dropped:
--   - Branch 1: steps_completed, total_steps
--   - Branch 2: steps_completed, total_steps, completed_at, the two in_progress
--               CASE arms, and the in_progress WHERE gate
--   - Outer query: timestamptz cast on ORDER BY (reverted to text sort)
--
-- This migration takes the P699 body verbatim and inserts actor_slug immediately
-- after actor_name in each branch. No other deviations from P699.
--
-- Authoritative reference: 20260415190000_p699_inbox_order_and_case_align.sql
...
BEGIN
  ...
  FROM (
    ...
    FROM (
      ...
      --   Includes in-progress (status = 'in_progress') and completed.
      --   Excludes 'sent'/'opened' (no recipient activity yet).
      SELECT jsonb_build_object(
        'type',            CASE
