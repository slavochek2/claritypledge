-- ============================================================================
-- P975: Restore the relationship-scope gate on seal_and_send_letter (4-arg overload)
--
-- diffed against: 20260618120000_p952_responses_mode.sql
--
-- SECURITY REGRESSION FIX.
--   P914 (20260610140000) added a relationship-scope gate to seal_and_send_letter
--   so a non-admin sender cannot resolve an arbitrary profile's email by UUID
--   (email-harvesting / unsolicited-letter oracle — receiver_profile_id is
--   caller-supplied via p_deliveries, and the function is SECURITY DEFINER so it
--   reads profiles.email bypassing RLS).
--
--   P952 (20260618120000) recreated the function from a pre-P914 base to add the
--   p_responses_mode parameter, and SILENTLY DROPPED the gate. The reachable
--   4-arg overload (the one the client calls — see letters-service.ts) has shipped
--   to prod WITHOUT the gate since then.
--
-- This migration re-applies the P914 gate on top of P952's current 4-arg body.
-- The full body is reproduced verbatim from P952 (idempotent CREATE OR REPLACE)
-- so the imageUrl (P819) and lead_count (P898) invariants are preserved — only
-- the v_is_admin declaration/fetch and the scope-gate RAISE are added back.
--
-- Regression guard: src/tests/p975-letter-scope-gate.test.ts asserts the latest
-- migration redefining seal_and_send_letter contains the scope gate, so a future
-- recreate-from-old-base cannot silently drop it again unnoticed.
-- ============================================================================
